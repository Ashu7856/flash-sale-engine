...
export default function Home() {
  ...
  const handleBuy = async (id: number) => {
    ...
    if (res.ok) {
      ...
      setProducts((prev) =>
        prev.map((p) =>
          p.id === id ? { ...p, stock: data.remaining_stock } : p
        )
      );
    }
  };
    ...
    <main className="min-h-screen bg-black text-white flex flex-col items-center p-8">
      ...
      {saleEnded ? (
        <div className="mt-10 text-center">
          ...
          <p className="text-zinc-500 mt-2">Check back for the next flash sale.</p>
        </div>
      ) : (
        <>
              ...
              return (
                <div key={p.id} className="bg-zinc-900 border border-zinc-700 rounded-2xl p-6 flex flex-col items-center w-72">
                  ...
                  <button
                    ...
                    disabled={saleEnded}
                    className="mt-4 w-full bg-red-600 hover:bg-red-700 disabled:bg-zinc-700 text-white text-xl py-3 rounded-xl"
                  >
                    Buy Now
                  </button>
                  {queue[p.id] && (
