          ...
          {STEPS.map((s, i) => (
            <div key={s} className="flex items-center gap-2 flex-1 last:flex-none">
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium
                ${i <= step ? "bg-violet-600 text-white" : "bg-zinc-100 text-zinc-400 dark:bg-zinc-800"}`}>
                {i < step ? "✓" : i + 1}
              </div>
              <span className={`text-sm ${i === step ? "text-zinc-900 dark:text-white font-medium" : "text-zinc-400"}`}>
                {s}
              </span>
              {i < STEPS.length - 1 && (
